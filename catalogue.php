<?php require ("header.php"); ?>
    ...
    function type3 ($b,$productsAll, $keyProduct, $forNew) {if ($b%5==0 || $b==1) {echo "<div class='rowCat'>
        ...
        for ($new=$forNew;;$new++) { if($keyProduct==0) {break;} $item = $productsAll[$new];
                    echo "<div class='cardCat'>
                            ...
                            <a class='cardCatR3C2' href='?item=$item[0]'>
    ...
    <div class='modal'>
        <?php 
        $item_get = isset($_GET['item'])?$_GET['item']:false;
        if ($item_get) {echo "<script> let cardCatR3C2x = document.getElementsByClassName('cardCatR3C2');
            cardCatR3C2[0].addEventListener('click', function () {modalShadow.style.display='block';
                modal.style.display='grid';} );
                              cardCatR3C2x[0].click(); </script>";
            $product = mysqli_fetch_all(mysqli_query($connect, "SELECT * FROM products WHERE id_product=$item_get"));
